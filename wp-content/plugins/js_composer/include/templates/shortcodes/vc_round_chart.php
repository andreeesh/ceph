...
$canvas_html = '<canvas class="vc_round-chart-canvas" ></canvas>';
$legend_html = '';
if ( $legend ) {
	foreach ( $data as $v ) {
		$color = is_array( $v['color'] ) ? current( $v['color'] ) : $v['color'];
		list($value, $label) = explode("%", $v['label']);
		$legend_html .= '<li><span style="background-color:' . $color . '"></span>
							 <span style="width:55px!important;font-size:20px!important;line-height:26px;font-weight:bold">'.$value.'% </span> 
							 <span style="width:auto!important">'.$label.'</span></li>';
	}
